...
def plot_latent_space(dim1=None, dim2=None, sample_dim=10, dir='outputs/latent_space/'):
    if dim1 is None:
...
def plot_all_latent_spaces(sample_dim=10, dir='outputs/latent_space/'):
    for i in range(config['model_params']['latent_dim']):
        for j in range(config['model_params']['latent_dim']):
            if i < j:
                plot_latent_space(i, j, sample_dim, dir)

def plot_all_latent_vectors(sample_dim=5, dir='outputs/latent_space/vectors/'):
    for i in range(config['model_params']['latent_dim']):
        plot_latent_space(i, i, sample_dim, dir)

def plot_random_latent_spaces(num_samples=20):
    for i in range(num_samples):
        plot_latent_space()

plot_latent_space(16, 15, 5)
#plot_all_latent_vectors()
